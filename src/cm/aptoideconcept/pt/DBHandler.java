...
//			c= database.query(DBStructure.TABLE_APK , new String[]{"*","max("+DBStructure.COLUMN_APK_VERCODE+")"}, null, null, DBStructure.COLUMN_APK_APKID, null, DBStructure.COLUMN_APK_NAME+" collate nocase",null);
	...
	public Cursor getInstalled() {
		Cursor c = null;
		try {
			c = database.rawQuery("select a.name, b._id, b.icon, a.vercode,b.repo_id from installed a INNER JOIN apk b ON a.apkid=b.apkid ",null);
			c.moveToFirst();
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public Cursor getUpdates() {
		...
		Cursor c = null;
		 try {
			c= database.rawQuery("select a.name, b._id,b.icon,b.vercode,b.repo_id from installed as a INNER JOIN apk b ON a.apkid=b.apkid and a.vercode < b.vercode",null);
			c.moveToFirst();
		} catch (Exception e) {
			e.printStackTrace();
		}
...
//		database.setTransactionSuccessful();
	...
	public Cursor getApkByCategory(String category) {
		...
		try {
			c= database.rawQuery("select b.name, b._id,b.icon,b.vercode,b.repo_id from category a, apk b where a._id=b._id and a.category2='"+category+"'",null);
		} catch (Exception e) {
			e.printStackTrace();
		}
