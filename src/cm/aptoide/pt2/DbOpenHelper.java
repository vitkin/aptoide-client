...
public class DbOpenHelper extends SQLiteOpenHelper {
	...
	public void onCreate(SQLiteDatabase db) {
		...
		db.execSQL("create table toprepo_extra (_id integer, top_delta text, iconspath text, category text);");
		...
		db.execSQL("create table itembasedapk (_id integer primary key, itembasedapkrepo_id integer, apkid text, name text, vercode integer, vername text, category2 text, downloads integer, rating text, icon text, md5 text, path text, size integer, parent_hashid integer);");
		db.execSQL("create table itembasedapk_hash (hash text, parent_hashid integer);");
